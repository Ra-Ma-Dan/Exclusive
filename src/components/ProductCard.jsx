    ...
    return (
        <div className='border-2 p-2 rounded-2xl flex flex-col hover:scale-[1.01] cursor-pointer delay-75 transition-all ease-linear'>
            ...
            <div className='flex justify-between items-center'>
                <Link to={`/product-info/${product.id}`}><button className='font-bold bg-gray-300 px-8 py-3 hover:bg-gray-500'>
                    view info</button></Link>
                <button onClick={() => {addToCart(product)}}
                className='bg-gray-300 font-bold px-8 py-3 hover:bg-gray-500'>Add to cart</button>
            </div>
        </div>
    )
}
